-- Resource Movement Management System Schema
...
CREATE TABLE IF NOT EXISTS resource_movement_requests (
  ...
  notes TEXT,
  reference_id TEXT, -- PO, Job Card, etc.
...
CREATE TABLE IF NOT EXISTS resource_movement_executions (
  ...
  vehicle_id TEXT, -- For fleet movements
  driver_id TEXT, -- For fleet movements
  route_plan TEXT,
...
-- 3. Movement Checkpoints (for tracking)
...
CREATE TABLE IF NOT EXISTS fleet_vehicles (
  ...
  capacity TEXT, -- e.g., "5 tons", "12 passengers"
...
CREATE TABLE IF NOT EXISTS fleet_drivers (
  ...
  specialized_certifications TEXT[], -- e.g., ['hazmat', 'passenger_transport']
  ...
  safety_rating DECIMAL(3,2) DEFAULT 5.00, -- 1.00 to 5.00
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS movement_routes (
  ...
  waypoints TEXT[], -- Array of intermediate locations
  estimated_distance DECIMAL(8,2), -- km
  estimated_duration INTEGER, -- minutes
  fuel_consumption DECIMAL(8,2), -- liters
...
CREATE TABLE IF NOT EXISTS movement_analytics (
  ...
  fuel_efficiency DECIMAL(8,2) DEFAULT 0, -- km/liter
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
SELECT 
  ...
  SUM(rme.distance_traveled) as total_distance,
  SUM(rme.fuel_consumed) as total_fuel,
  AVG(rme.distance_traveled / NULLIF(rme.fuel_consumed, 0)) as avg_fuel_efficiency,
  SUM(rme.actual_cost) as total_cost
...
SELECT 
  ...
  SUM(rme.distance_traveled) as distance_this_month,
  AVG(rme.actual_duration) as avg_trip_duration
