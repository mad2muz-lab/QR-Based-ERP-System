-- Fix for purchase_requests migration
-- This script ensures the purchase_requests table is created properly
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'purchase_requests') THEN
        ...
        RAISE NOTICE 'Purchase requests tables created successfully';
    ELSE
        RAISE NOTICE 'Purchase requests tables already exist';
    END IF;
END $$;
