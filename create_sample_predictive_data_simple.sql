-- Create Sample Data for Predictive Stocking System (Simplified)
-- This script creates sample predictions, alerts, and accuracy tracking data
-- Works with any existing equipment and materials

-- 1. Insert sample predictions (using any equipment and materials)
...
SELECT 
  ...
  jsonb_build_object(
    'pm_frequency_days', COALESCE(e.pm_frequency_days, 30),
    'total_pm_logs', 5,
    'average_usage_per_pm', 1,
    'equipment_type', e.type,
    'pm_class', COALESCE(e.pm_class, 'Class A')
...
WHERE (
  ...
  OR COALESCE(e.pm_frequency_days, 30) <= 30
)
LIMIT 5;
