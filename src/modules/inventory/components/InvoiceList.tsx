...
import { Plus, Eye, Trash2, DollarSign, Search, AlertCircle } from 'lucide-react';
import { Invoice, getInvoices, deleteInvoice } from '../../../utils/invoiceService';
...
const InvoiceList: React.FC = () => {
  ...
  const handleConvertProforma = (proformaId: string) => {
    navigate(`/inventory/invoice/new/${proformaId}`);
  };
    ...
    <div style={{ background: 'white', borderRadius: '16px', border: '2px solid #e2e8f0', overflow: 'hidden' }}>
      <div style={{ padding: '20px 24px', borderBottom: '2px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <DollarSign style={{ width: '22px', height: '22px', color: '#002e17' }} />
          <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#0f172a', margin: 0 }}>Invoices ({invoices.length})</h3>
        </div>
        <button onClick={() => navigate('/inventory/invoice/new')} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', background: '#002e17', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '700', fontSize: '15px', cursor: 'pointer' }}>
          <Plus style={{ width: '18px', height: '18px' }} /> New Invoice
        </button>
      </div>
      ...
      <div style={{ maxHeight: '600px', overflowY: 'auto' }}>
        {filtered.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '16px', color: '#64748b', fontWeight: '600' }}>No invoices found</p>
            <p style={{ fontSize: '14px', color: '#94a3b8' }}>Create your first invoice</p>
          </div>
        ) : (
              ...
              {filtered.map(invoice => {
                ...
                return (
                  <tr key={invoice.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '14px 16px', fontSize: '14px', fontWeight: '600', color: '#0f172a' }}>{invoice.invoiceNumber}</td>
                    <td style={{ padding: '14px 16px', fontSize: '14px', color: '#475569' }}>{invoice.customerName}</td>
                    <td style={{ padding: '14px 16px', fontSize: '14px', color: '#6b7280' }}>{invoice.issueDate}</td>
                    <td style={{ padding: '14px 16px', fontSize: '14px', fontWeight: '700', color: '#0f172a', textAlign: 'right' }}>SAR {invoice.grandTotal.toFixed(2)}</td>
                    <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                      <span style={{ padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '700', background: statusColor.bg, color: statusColor.color }}>
                        {invoice.status}
                      </span>
                    </td>
                    <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                        <button onClick={() => navigate(`/inventory/invoice/${invoice.id}`)} style={{ padding: '8px', background: '#eff6ff', color: '#2563eb', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                          <Eye style={{ width: '16px', height: '16px' }} />
                        </button>
                        <button onClick={() => handleDelete(invoice.id)} style={{ padding: '8px', background: '#fef2f2', color: '#dc2626', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
