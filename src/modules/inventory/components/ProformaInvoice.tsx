...
const ProformaForm: React.FC<{ editId?: string }> = ({ editId }) => {
  ...
  const [items, setItems] = useState<Partial<ProformaItem>[]>([
  ...
  useEffect(() => {
    if (editId) {
      ...
      const existing = proformas.find(p => p.id === editId);
      if (existing) {
        setCustomerName(existing.customerName);
        setCustomerAddress(existing.customerAddress || '');
        setCustomerVatNumber(existing.customerVatNumber || '');
  ...
  const calculateItem = (item: Partial<ProformaItem>) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const proformaItems: ProformaItem[] = items.map((item, idx) => {
      const calc = calculateItem(item);
      return {
        id: `item-${idx}`,
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px', background: 'white', borderRadius: '16px', border: '2px solid #e2e8f0', padding: '28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
        ...
        <div>
          <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#0f172a', margin: 0 }}>{editId ? 'Edit Proforma' : 'New Proforma Invoice'}</h2>
          <p style={{ fontSize: '14px', color: '#6b7280', margin: '2px 0 0 0' }}>Create a proforma invoice for your customer</p>
        </div>
      </div>
      ...
      {saved && (
        <div style={{ padding: '14px 18px', borderRadius: '12px', marginBottom: '20px', fontSize: '15px', fontWeight: '600', background: '#d1fae5', color: '#065f46', border: '2px solid #6ee7b7' }}>
          Proforma created successfully! Redirecting...
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
          ...
          <div>
            ...
            <input type="date" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} style={{ width: '100%', padding: '12px 16px', border: '2px solid #d1d5db', borderRadius: '10px', fontSize: '15px', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
        ...
        <button type="submit" style={{ width: '100%', padding: '14px', background: '#002e17', color: 'white', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
          <Save style={{ width: '18px', height: '18px' }} /> Save Proforma
        </button>
      </form>
    </div>
  );
