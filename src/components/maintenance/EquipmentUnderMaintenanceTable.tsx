...
interface EquipmentUnderMaintenanceTableProps {
  enrichedEquipment: any[];
}

const EquipmentUnderMaintenanceTable: React.FC<EquipmentUnderMaintenanceTableProps> = ({ enrichedEquipment }) => (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {enrichedEquipment.map((eq) => (
            <tr key={eq.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full`}>{eq.status}</span>
              </td>
...
export default EquipmentUnderMaintenanceTable; 