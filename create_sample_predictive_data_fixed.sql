-- Create Sample Data for Predictive Stocking System (Fixed for TEXT material_id)
...
SELECT 
  ...
  COALESCE(m.stock_quantity, 0) as current_stock_level,
  CASE 
...
SELECT 
  ...
  e.id as equipment_id,
  CASE 
    WHEN COALESCE(m.stock_quantity, 0) = 0 THEN 'low_stock'
    WHEN COALESCE(m.stock_quantity, 0) <= 2 THEN 'low_stock'
    ELSE 'high_confidence_prediction'
  END as alert_type,
  CASE 
    WHEN COALESCE(m.stock_quantity, 0) = 0 THEN 
      'Critical: No stock available for ' || m.name || ' needed by ' || e.name
    WHEN COALESCE(m.stock_quantity, 0) <= 2 THEN 
      'Low stock alert: Only ' || m.stock_quantity || ' units of ' || m.name || ' remaining'
    ELSE 
  ...
  CASE 
    WHEN COALESCE(m.stock_quantity, 0) = 0 THEN 'critical'
    WHEN COALESCE(m.stock_quantity, 0) <= 2 THEN 'high'
    ELSE 'medium'
  END as priority,
  CASE 
    WHEN COALESCE(m.stock_quantity, 0) = 0 THEN 
