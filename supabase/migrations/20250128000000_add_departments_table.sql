...
-- This migration creates the departments table in Supabase

-- Create departments table
CREATE TABLE IF NOT EXISTS departments (
  id TEXT PRIMARY KEY DEFAULT ('dept-' || replace(gen_random_uuid()::text, '-', '')),
...
CREATE POLICY "Allow all operations on departments"
  ON departments FOR ALL TO anon, authenticated USING (true) WITH CHECK (true);
...
INSERT INTO departments (id, name, description, type, created_at, last_updated) VALUES
  ('dept-construction', 'Construction', 'Construction and building operations', 'operations', NOW(), NOW()),
  ('dept-operations', 'Operations', 'Daily operations and maintenance', 'operations', NOW(), NOW()),
  ('dept-maintenance', 'Maintenance', 'Equipment maintenance and repair services', 'technical', NOW(), NOW()),
  ('dept-engineering', 'Engineering', 'Engineering and technical services', 'technical', NOW(), NOW()),
  ('dept-safety', 'Safety', 'Health, safety, and environmental', 'compliance', NOW(), NOW()),
  ('dept-procurement', 'Procurement', 'Material procurement and supply chain management', 'support', NOW(), NOW()),
  ('dept-admin', 'Administration', 'Administrative and support services', 'support', NOW(), NOW())
...
COMMENT ON TABLE departments IS 'Departments table for organizational structure';
