-- Apply separate log tables migration safely
-- This migration creates the new log tables if they don't exist
...
-- Create RLS policies for employee_logs (drop if exists first)
DROP POLICY IF EXISTS "Allow all operations on employee_logs" ON employee_logs;
...
-- Create RLS policies for equipment_logs (drop if exists first)
DROP POLICY IF EXISTS "Allow all operations on equipment_logs" ON equipment_logs;
...
-- Create RLS policies for material_logs (drop if exists first)
DROP POLICY IF EXISTS "Allow all operations on material_logs" ON material_logs;
